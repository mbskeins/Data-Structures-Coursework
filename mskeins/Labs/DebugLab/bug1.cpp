...
int main() {
    ...
    //std::cout << "Printing Main Array: " << std::endl;
...
void UniqRandInt(int max, int n, int result[]) {
    ...
    while(cntr < n) {
        ...
        r = r % (max + 1);
	//std::cerr << "value of random number :" << r << std::endl;
	//std::cerr << "Value of cntr during while loop (rand func): "
	//  << cntr << std::endl;
...
bool inArray(int array[], int arrSize, int x) {
    for (int i = 0; i < arrSize; ++i) {
      //std::cerr << "value of I during for loop (checking inArray):  " << i  << std::endl;
        if (array[i] == x) {
            return true;
        }
    }
